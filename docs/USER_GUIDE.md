# Crane Remote Control â€” User Guide

A complete beginner's guide: what this program is, how to start it, how to drive the
crane, and what every button does. No programming knowledge needed for chapters 1â€“9.

---

## 1. What this program is

It is the **software half of a crane remote control** â€” the same kind of handheld radio
...
2. **Applies safety rules** to those wishes â€” this is the important part.
...
crane over a serial cable instead â€” nothing else about the program changes.

> âš ï¸ **Safety note.** This is a development platform, simulator and operator interface.
...
### Option A â€” Install it (easiest)

1. Go to `crane-ui\build\jpackage\`
2. Double-click **`CraneRemoteControl-3.4.3.msi`**
3. Click through the installer (no admin rights needed â€” it installs for you only)
4. Start it from the Start menu: **Crane Remote Control**

You do **not** need Java installed. The installer bundles everything.

### Option B â€” Portable (no install)

Copy the whole folder `crane-ui\build\jpackage\CraneRemoteControl\` anywhere â€” a USB
stick works â€” and double-click `CraneRemoteControl.exe`.

### Option C â€” From source (for development)
...
### Left â€” CONTROLS (what you command)
...
### Right â€” STATUS (what the crane is actually doing)
...
- **SAFETY**: three lamps â€” E-STOP latched, deadman held, watchdog tripped.
...
3. **While still holding Space**, press `W`. The main boom rises. Let go of `W` â€” it
   stops.
...
zero when you release them. Keyboard and slider are combined â€” whichever asks for more
movement wins.
...
1. **E-STOP latches.** Once tripped, every command is forced to zero *immediately*, and
   stays zero. Letting go of the button is not enough â€” it is a latch, like a real
...
3. **Deadman released â‡’ everything stops.** Not frozen instantly (that would shock the
   hydraulics and swing the load), but ramped to zero fast and under control.
4. **Watchdog.** If the software stops receiving fresh commands for 250 milliseconds â€”
   a crash, a frozen program, an unplugged cable â€” it assumes the worst and stops the
   crane, exactly as if you had let go of the deadman.
5. **Clamping and ramping.** Commands are limited to the Â±1.0 range, and how *fast* a
...
> the watchdog working correctly before the first command arrives â€” not a fault.
...
- **SMOOTHING** â€” rounds off starts and stops (an S-curve instead of a step). The crane
  feels like an experienced operator is at the levers.
- **ANTI-SWAY** â€” the crane watches the load swinging on the rope and adds tiny slew
  ...
  ANTI-SWAY on and repeat â€” it settles roughly twice as fast.
- **FOLD TO TRANSPORT** â€” the automatic "pack up and go home" sequence. It moves one axis
  at a time in a safe order: retract the extension â†’ hook up â†’ fold the jib â†’ lower the
  boom â†’ centre the slew. **You must still hold Space** while it runs; releasing pauses
...
- Next to the hook, a live readout: `out 9.0 m / h 2.4 m` â€” how far the hook is from the
  crane's centre of rotation, and how high above the ground.

### 3D â€” the harbour scene

The crane on its truck at a quay, with water, a moored boat and a sun. Choose from the
right panel:

- **Camera: Orbit** â€” free look. Drag to turn around the crane, scroll to zoom.
- **Camera: Cab** â€” from the driver's seat, looking at the load.
- **Camera: Hook** â€” straight down from above the hook. The best view for placing a load
  precisely.
- **Camera: Follow load** â€” behind and above the load, tracking it as you slew.

- **Load: None / Pallet / Container / Small boat** â€” hangs from the hook, swings with it,
...
The load is drawn in **both** views â€” the 2D view shows it as a labelled silhouette under
the hook â€” so switching between 2D and 3D never changes what is on the hook.
...
carry a load â€” exactly how a real loader crane is arranged.
...
**RELEASE LOAD** unhooks the load where it hangs â€” in reality the ground crew does this,
...
- The crane is **locked out completely** â€” every axis demand is forced to zero while
  the mode is on. You are either on the remote or behind the wheel, never both.
- Drive with the **arrow keys**: `â†‘` throttle, `â†“` brake and reverse, `â†`/`â†’` steer.
...
**WEATHER** sets a wind speed (0â€“20 m/s) and the compass point it blows *from*. Wind
...
cab, the deck, the ground, or a load standing in the way â€” the same idea real cranes call
...
If an axis stops and the position is nowhere near its limit, this is usually why â€” check
...
happened during a test, or â€” later â€” train an automatic control system.
...
## 10. Driving a different crane â€” profiles

This is what makes the software "universal": **a crane is a data file, not code.**

The PROFILE selector lists:

- *Demo Knuckle-Boom (5-axis)* â€” the built-in default,
- *Compact Loader (3-axis)* â€” a small crane with no jib and no extension (notice the
  `E`/`D` and `R`/`F` keys do nothing, and the panels have fewer rows),
- *Heavy Knuckle-Boom (5-axis)* â€” bigger, slower, longer reach,
...
| `commandRampRate` | how fast the command may change â€” smaller = gentler machine |
...
- **Simulator** â€” the built-in virtual crane (default).
- **Serial: COM3, COM4, â€¦** â€” a real crane connected to that port, speaking the protocol
...
```
    YOU (keyboard, sliders)
          â”‚  "I want the boom to go up"
          â–¼
    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
    â”‚  Operator interface   â”‚   crane-ui   â€” draws everything, reads your input
    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                â”‚  a command: {boom: +1.0, deadman held}
                â–¼
    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
    â”‚  Assists (optional)   â”‚   smoothing Â· anti-sway Â· auto-fold
    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                â–¼
    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
    â”‚  SAFETY LAYER         â”‚   crane-core â€” the six rules. Has the last word.
    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                â”‚  a safe command
                â–¼
    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
    â”‚  Driver (the "port")  â”‚   simulator today Â· serial cable to real hardware later
    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                â”‚  positions and speeds coming back
                â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º back up to the screen
```

The middle box runs on its own clock, **50 times every second**, whether or not you touch
anything. Each cycle it: takes your latest command â†’ runs the assists â†’ runs the safety
rules â†’ sends the result to the crane â†’ reads back where the crane now is â†’ hands that to
the screen to draw.
...
| Still nothing, red banner | E-STOP is latched. Click **RESET** â€” with all controls at zero and Space released. |
...
| Some keys do nothing | That crane has no such axis â€” the compact profile has no jib or extension. |
...
| What | Where |
|---|---|
| Installer | `crane-ui\build\jpackage\CraneRemoteControl-3.4.3.msi` |
...
- [VISION.md](VISION.md) â€” what this product is for and who might buy it
- [ARCHITECTURE.md](ARCHITECTURE.md) â€” the technical design
- [PROTOCOL.md](PROTOCOL.md) â€” the wire protocol for real hardware
- [BACKLOG.md](BACKLOG.md) â€” what is done and what is planned
